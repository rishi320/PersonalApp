...
function App() {
  ...
  const [isLoggedIn, setLoggedIn] = useState(loggedIn==="true"?true:false);

  let signedUp = localStorage.getItem("IS_SIGNED_UP");
  const [isSignedUp, setSignedUp] = useState( signedUp==="true"?true:false);
    ...
    <AppContainer hideNavigation={true}>
       <div className="main-container">
         <div className="navbar-container">
          {(isLoggedIn === true ? (<NavigationSetup userDetails = {userDetails}></NavigationSetup>) 
                               : ((!isSignedUp) ? (<SignIn setSigned={setDetails} newUser={newUser} modifySigned = {setNewDetails}></SignIn>)
                                                      :(<SignUp></SignUp>)))}
         </div>
       </div>
